...
-- One row per dispute against a captured authorization, keyed by the
-- cardproc simulator's own chargeback ID so a redelivered "opened" webhook
-- is a no-op rather than a second provisional credit.
