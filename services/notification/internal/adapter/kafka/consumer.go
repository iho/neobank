...
func NewConsumer(brokers, groupID string, ingest *usecase.IngestEventUseCase, logger *slog.Logger) *Consumer {
	if groupID == "" {
		groupID = "notification-service"
	}
	return &Consumer{brokers: brokers, groupID: groupID, ingest: ingest, logger: logger}
}
...
func (c *Consumer) consumeTopic(ctx context.Context, topic string) error {
	...
	for {
		...
		} else if err := c.ingest.Execute(ctx, envelope); err != nil {
			c.logger.Warn("ingest failed", "topic", topic, "type", envelope.EventType, "error", err)
		}
