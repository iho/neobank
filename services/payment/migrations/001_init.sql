...
CREATE TABLE IF NOT EXISTS payment.transfers (
...
CREATE INDEX IF NOT EXISTS idx_transfers_sender
    ON payment.transfers (sender_user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_transfers_recipient
    ON payment.transfers (recipient_user_id, created_at DESC);
...
CREATE TABLE IF NOT EXISTS payment.saga_instances (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
