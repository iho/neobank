...
CREATE TABLE rails.inbound_transfers (
    id          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_rails_inbound_transfers_account
    ON rails.inbound_transfers (account_id, created_at);

CREATE INDEX idx_rails_inbound_transfers_created_at
    ON rails.inbound_transfers (created_at);

CREATE TABLE rails.outbound_payments (
    id                 UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    account_id         UUID NOT NULL REFERENCES rails.accounts(id),
    amount             NUMERIC(20,2) NOT NULL CHECK (amount > 0),
    currency           CHAR(3) NOT NULL,
...
CREATE INDEX idx_rails_webhook_deliveries_due
