...
func (q *Queries) ClaimDueDeliveries(ctx context.Context, arg ClaimDueDeliveriesParams) ([]RailsWebhookDelivery, error) {
	rows, err := q.db.Query(ctx, claimDueDeliveries, arg.Now, arg.LimitVal)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []RailsWebhookDelivery{}
	for rows.Next() {
		var i RailsWebhookDelivery
		if err := rows.Scan(
			&i.ID,
			&i.Url,
			&i.EventType,
...
func (q *Queries) GetDelivery(ctx context.Context, id uuid.UUID) (RailsWebhookDelivery, error) {
...
func (q *Queries) ListDeliveries(ctx context.Context, limitVal int32) ([]RailsWebhookDelivery, error) {
