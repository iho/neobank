...
func (uc *SubmitKYCUseCase) Execute(ctx context.Context, in SubmitKYCInput) (SubmitKYCOutput, error) {
	...
	// Sanctions/PEP screening is an independent, instant hard-stop — it
	// doesn't need the identity vendor's opinion to reject.
	...
	// Screening passed — hand off to the KYC vendor for identity
	// verification. The verdict (approved/rejected/manual_review) arrives
	// later via webhook (ProcessKYCVerdictUseCase), not in this response.
	if uc.vendor == nil {
		return SubmitKYCOutput{}, fmt.Errorf("kyc vendor unavailable")
	}
	applicant, err := uc.vendor.SubmitApplicant(ctx, in.UserID, in.FullName, in.DateOfBirth, in.CountryCode)
	if err != nil {
		return SubmitKYCOutput{}, fmt.Errorf("kyc vendor: %w", err)
	}
	...
	if err := uc.tx.Run(ctx, func(ctx context.Context, tx pgx.Tx) error {
		...
		if err := kycTx.SetVendorApplicant(ctx, caseID, applicant.ID); err != nil {
			return err
		}
		return uc.audit.WithTx(tx).Record(ctx, audit.Entry{
			EntityType: "kyc_case",
			...
			FromStatus: string(domain.KYCStatusPending),
			ToStatus:   string(domain.KYCStatusPending),
			Metadata:   map[string]any{"vendor_applicant_id": applicant.ID},
		})
	}); err != nil {
		return SubmitKYCOutput{}, err
	}
