CREATE TABLE IF NOT EXISTS "user".saga_alerts (
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_user_saga_alerts_active
    ON "user".saga_alerts (saga_instance_id)
    WHERE alert_status IN ('open', 'investigating');

CREATE INDEX IF NOT EXISTS idx_user_saga_alerts_open
    ON "user".saga_alerts (alert_status, last_seen_at DESC);